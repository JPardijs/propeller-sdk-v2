...
const Valueset_1 = require("../type/Valueset");
const ValuesetResponse_1 = require("../type/ValuesetResponse");
const ValuesetItemResponse_1 = require("../type/ValuesetItemResponse");
/**
 Service class for Valueset-related GraphQL operations
 */
class ValuesetService extends BaseService_1.BaseService {
    /**
    ...
    async getValueset(id) {
        ...
        return new Valueset_1.Valueset(result.data.valueset);
    }
    /**
     ...
     */
    async getValuesets(input) {
        const variables = { input };
        const result = await this.executeQuery('valuesets', variables);
        return new ValuesetResponse_1.ValuesetResponse(result.data.valuesets);
    }
    /**
    ...
    async createValueset(input) {
        ...
        return new Valueset_1.Valueset(result.data.valuesetCreate);
    }
    /**
     ...
     */
    async updateValueset(input) {
        const variables = { input };
        const result = await this.executeMutation('valuesetUpdate', variables);
        return new Valueset_1.Valueset(result.data.valuesetUpdate);
    }
    /**
     Deletes a valueset
     ...
     */
    async getValuesetItems(input) {
        const variables = { input };
        const result = await this.executeQuery('valuesetItems', variables);
        return new ValuesetItemResponse_1.ValuesetItemResponse(result.data.valuesetItems);
    }
    /**
...
//# sourceMappingURL=ValuesetService.js.map